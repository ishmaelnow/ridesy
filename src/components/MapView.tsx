...
export default function MapView({ showDriver }: { showDriver?: boolean }) {
  ...
  const { ride, status, userLocation, driverLocation } = useRide();
  ...
  // ─── Rider location blue dot (live via watchPosition) ─────────────────────
  useEffect(() => {
  ...
  // ─── Driver marker — uses REAL driverLocation from context ────────────────
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !mapReady) return;
    ...
    if (showDriver && driverLocation) {
      ...
      const driverIcon = L.divIcon({
        className: "",
        html: `<div style="width:28px;height:28px;background:hsl(220,15%,6%);border:3px solid white;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.4);">
      ...
      // During active ride, keep camera centred on the driver
